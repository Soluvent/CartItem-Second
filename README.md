...
## `📝 Завдання 📝`
- [x] **Створіть клас "Товар" (Item)**
  - [x] А) Клас «Товар» має містити такі поля: найменування (рядок), ціна (float).
  - [x] Б) Клас «Товар» повинен містити конструктор, який приймає два параметри: найменування та початкову ціну товару.
  - [x] В) Клас «Товар» повинен мати такі публічні методи: підвищення ціни на певний відсоток (значення відсотка типу float передається як аргумент методу); зниження ціни певний відсоток (значення відсотка типу float передається як аргумент методу).
  - [x] Г) У класі має бути реалізована перевірка ціни на негативне значення. Якщо конструкторі передається негативне значення ціни, чи результаті зниження ціни на >100% ціна стає негативною, вона має бути примусово встановлено 0.

- [x] **Створіть клас «Кошик» (Cart)**
...
- [x] **У методі Main необхідно створити об'єкт класу «Кошик» з деякою максимальною кількістю елементів у стеку.**
...
- [x] **Перепишіть клас Cart, використовуючи замість стека чергу без зміни інтерфейсу класу.**
...
class Cart {
    ...
    // Забираємо товар з черги (dequeue)
    public Item removeItem() {
        if (isEmpty()) {
            System.out.println("Помилка: Кошик порожній!");
            return null;
